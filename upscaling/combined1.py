...
def create_overlay_video(dir, output_video, fps=10, temporal_window_size = 5, h = 10):
    ...
    for i in range(len(images)):
        ...
        start_idx = max(0, i - temporal_window_size // 2)
        end_idx = min(len(images), i + temporal_window_size // 2 + 1)
        
        img_stack = np.stack(images[start_idx:end_idx], axis=3)

        denoised_image = cv2.fastNlMeansDenoisingMulti(
            img_stack, i - start_idx, 7, [1.0], None, 7, 39
        )
